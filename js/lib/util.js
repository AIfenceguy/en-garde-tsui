...
export function el(tag, attrs = {}, children = []) {
    ...
    for (const [k, v] of Object.entries(attrs || {})) {
        ...
        else if (k === 'for' || k === 'htmlFor') node.setAttribute('for', v);
        else if (k in node && typeof node[k] !== 'function') {
            // Some reflected DOM properties (e.g. HTMLInputElement.list, .form,
            // .labels, .control) are read-only getters. Trying to assign throws.
            // Fall back to setAttribute when direct assignment is rejected.
            try { node[k] = v; }
            catch (_) { node.setAttribute(k, v === true ? '' : v); }
        }
