-- The return follows the last day the family actually fences, not the last
-- day of the tournament; season-made watches that are not booked take their
-- date windows from the schedule on every sync. Applied 2026-09-11.
...
begin
  ...
  for t in
    select se.tournament,
           ...
           max(se.start_date) as last_date,          -- the last day someone in the family fences
           max(se.city) as city,
  ...
  loop
    ...
    if w_id is null then
      ...
      -- The schedule is the source of truth for the dates while nothing is
      -- booked; hours, stops and airports stay the parent's to set.
      update flight_watches set
        is_active = true,
        depart_date = t.first_date - 1,
        depart_window_start = t.first_date - 2,
        depart_window_end = t.first_date,
        return_date = t.last_date,
        return_window_start = t.last_date,
        return_window_end = t.last_date + 1,
